...
class ArrQueue {
 ...
 public:
  ...
  /**
   * @brief 入队函数，用于在数据队列的尾部（即arr(end) 位置）存入参数n
   * 中传递的数据，并更新size 和end 以保持正确的队列状态
   *
   * @param n
   * @return true 成功
  ...
  /**
   * @brief 出队函数，用于弹出数据队列的头部（即arr(front)
   * 位置）并将其值保存到引用参数n 中，如操作成功，则更新size 和front
   * 以保持正确的队列状态
   *
   * @param n
   * @return true 成功
  ...
  /**
   * @brief 重载操作符+ ，使用该操作符可对参数n
   * 指定的整数执行入队操作。入队操作后，将当前数据队列作为操作结果返回值，以便支持操作符+
   * 的链式操作；
   *
   * @param n
   * @return ArrQueue&
  ...
  /**
   * @brief 重载操作符-
   * ，使用该操作符可对数据队列执行出队操作，并将弹出的队首元素存入引用参数n
   * 中。出队操作后，将当前数据队列作为操作结果返回值，以便支持操作符-
   * 的链式操作；
   *
   * @param n
   * @return ArrQueue&
  ...
  /**
   * @brief 重载操作符<<
   * ，使用该操作符可对数据队列中保存的数据内容进行打印输出，打印时对数据项不执行出队操作
   *
   * @param os
   * @param q
   * @return std::ostream&
