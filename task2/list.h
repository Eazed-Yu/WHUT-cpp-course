 ...
 * data : 用于保存当前节点中的整型数据
 * prev :
 * 指向链表中的前置邻接节点（左节点）的指针，如果当前节点没有前置邻接节点，则该数据成员应为空值；
 * next :
 * 指向链表中的后置邻接节点（右节点）的指针，如果当前节点没有后置邻接节点，则该数据成员应为空值；
 */
struct Node {
  int data;
  Node *prev;
...
class List {
 ...
 public:
  ...
  /**
   * @brief
   * 用于从双向链表上删除其头节点（最左节点），并将该节点中的数据保存到参数n
   *
   * @param n 保存到参数n
   * @return true 操作成功
   * @return false 链表为空
  ...
  /**
   * @brief
   * 用于从双向链表上删除其尾节点（最右节点），并将该节点中的数据保存到参数n
   *
   * @param n 保存到参数n
   * @return true 操作成功
   * @return false 链表为空
