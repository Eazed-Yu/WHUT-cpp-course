...
public:
    ...
    /**
     * @brief 压栈函数，用于在数据栈的栈顶（即arr(size) 位置）压入参数n 中传递的数据，并更新size
     * 
     * @param n 
     * @return true 添加成功
     * @return false 容量已满
    ...
    /**
     * @brief 出栈函数，用于弹出数据栈顶的元素（即arr(size-1) 位置）并将其值保存到引用参数n 中
     * 
     * @param n 
     * @return true 出栈成功
     * @return false 栈为空
...
public:
    ...
    bool is_full() const { return this->lis.length() == LIST_MAX; }            // 检查栈是否已满
